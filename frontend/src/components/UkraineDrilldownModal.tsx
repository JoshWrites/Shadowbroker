...
interface Props {
    onClose: () => void;
    ukraineDbRange: { earliest: number | null; latest: number | null };
    setUkraineTimeOffset: React.Dispatch<React.SetStateAction<number | null>>;
...
function CalendarPicker({ value, onChange, minTs, maxTs, label }: CalendarPickerProps) {
    ...
    return (
        <div className="flex flex-col gap-1 flex-1 relative" ref={ref}>
            ...
            <button
                onClick={() => setOpen(o => !o)}
                className="flex items-center gap-2 bg-[var(--bg-primary)] border border-[var(--border-primary)] hover:border-blue-700/60 rounded px-2 py-1.5 text-[10px] font-mono text-[var(--text-primary)] focus:outline-none focus:border-blue-700 transition-colors text-left"
            >
                <Calendar size={10} className="text-blue-400/60 flex-shrink-0" />
            ...
            <AnimatePresence>
                {open && (
                    <motion.div
                        className="absolute top-full left-0 mt-1 z-[10100] w-[220px] bg-[var(--bg-secondary)] border border-blue-900/40 rounded-lg shadow-[0_8px_30px_rgba(0,0,0,0.6)] overflow-hidden"
                    ...
                    >
                        {/* Month nav */}
                        <div className="flex items-center justify-between px-3 py-2 border-b border-blue-900/20">
                                        ...
                                        className={[
                                            ...
                                            isSelected
                                                ...
                                                : outOfRange
                                                    ...
                                                    : 'text-[var(--text-secondary)] hover:bg-blue-950/40 hover:text-blue-400 cursor-pointer',
                                        ].join(' ')}
                                    >
                                        {i + 1}
                                    </button>
                        ...
                        {/* Time picker */}
                        <div className="flex items-center gap-1 px-3 py-2 border-t border-blue-900/20">
                            ...
                            <input
                                ...
                                className="w-9 bg-[var(--bg-primary)] border border-[var(--border-primary)] rounded px-1 py-0.5 text-[10px] font-mono text-[var(--text-primary)] text-center focus:outline-none focus:border-blue-700"
                            />
...
export default function UkraineDrilldownModal({ onClose, ukraineDbRange, setUkraineTimeOffset, ukraineTimeOffset }: Props) {
    const earliest = ukraineDbRange.earliest;
    const latest = ukraineDbRange.latest ?? Math.floor(Date.now() / 1000);
    ...
    useEffect(() => {
        ...
        countTimerRef.current = setTimeout(() => {
            ...
            fetch(`${BACKEND_DIRECT}/api/ukraine-alerts/history?from_ts=${rangeStart}&until_ts=${rangeEnd}`)
                .then(r => r.json())
    ...
    useEffect(() => {
        ...
        setUkraineTimeOffset(offsetMins === 0 ? null : offsetMins);
    }, [scrubTs, setUkraineTimeOffset, nowSec]);
    ...
    return (
        <AnimatePresence>
            ...
            >
                ...
                <motion.div
                    className="relative w-[520px] bg-[var(--bg-secondary)] border border-blue-900/40 rounded-xl shadow-[0_0_40px_rgba(59,130,246,0.1)] overflow-visible"
                ...
                >
                    {/* Header */}
                    <div className="flex items-center justify-between px-5 py-3 border-b border-blue-900/30 bg-blue-950/20 rounded-t-xl">
                        <div className="flex items-center gap-2">
                            <div className="w-2 h-2 rounded-full bg-blue-500 animate-pulse" />
                            <span className="text-[11px] font-mono tracking-[0.25em] text-blue-400">UKRAINE ALERTS — ARCHIVE DRILL-DOWN</span>
                        </div>
                    ...
                    <div className="px-5 py-4 flex flex-col gap-4">
                        ...
                        <div className="flex items-center gap-4 text-[10px] font-mono">
                            ...
                            <div className="flex flex-col gap-0.5">
                                ...
                                <span className={`text-blue-400 ${countLoading ? 'opacity-50' : ''}`}>
                        ...
                        <div className="flex flex-col gap-2">
                            <div className="flex items-center justify-between">
                                ...
                                <span className="text-[10px] font-mono text-blue-400">{scrubLabel}</span>
                            </div>
                            <div className="flex items-center gap-3">
                                <button
                                    onClick={() => setPlaying(p => !p)}
                                    className="w-7 h-7 flex items-center justify-center rounded border border-blue-500/40 text-blue-400 hover:bg-blue-950/40 transition-colors flex-shrink-0"
                                >
                        ...
                        <div className="flex items-center justify-between pt-1 border-t border-[var(--border-primary)]">
                            ...
                            <button
                                onClick={() => { setUkraineTimeOffset(null); onClose(); }}
